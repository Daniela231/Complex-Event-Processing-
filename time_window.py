...
def externally_last_time_observer(key, col, time):
    ...
    """
    all_dfs[key].dataframe = all_dfs[key].dataframe.append(last_event())
    all_dfs[key].dataframe = all_dfs[key].dataframe[all_dfs[key].dataframe[col] > np.datetime64('now') - time]


def externally_last_time(col, weeks=0, days=0, hours=0, minutes=0, seconds=0, milliseconds=0, microseconds=0, nanoseconds=0):
    """
    Returns the dataframe looking back a given time into the past from the current system time
    :param weeks: number of weeks into the past
    :param days: number of days into the past
    ...
    try:
        ...
        all_dfs[key] = DataframeManager()
        all_dfs[key].dataframe = all_dfs["StockTick"].dataframe[all_dfs["StockTick"].dataframe[col] > now - time]
        all_dfs[key].observers.update({externally_last_time_observer : [key, col, time]})
